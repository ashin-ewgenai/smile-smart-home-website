...
function toMillisFlexible(v: any): number | null {
  try {
...
function extractExpiryCandidates(data: any): number[] {
  const out: number[] = [];
  try {
    const pushIfValid = (v: any) => {
...
export const onUserDeviceWriteWarrantyNotify = functions.firestore
  ...
  .onWrite(async (change, context) => {
    try {
      ...
      if (!data) return;
      const uid: string | undefined = (data as any).uid;
      if (!uid) return;
      const deviceId = context.params.docId as string;
      const deviceName = (data as any)?.deviceName || (data as any)?.name || "Your device";
      ...
      for (const t of candidates) {
        if (t <= now || t <= in30Days) {
          if (chosen == null) chosen = t; else chosen = Math.min(chosen, t);
        }
      }
    ...
    try {
      ...
      userDevicesSnap.forEach((d) => {
        try {
          ...
          const candidates = extractExpiryCandidates(data);
          for (const t of candidates) {
            if (t <= nowMs) { expiredUids.add(uid); break; }
          }
        } catch {}
      });
...
export const getUserDeviceDetails = functions.https.onCall(async (data, context) => {
  try {
    if (!context.auth) {
    ...
    const normalizeWarranty = (v: any): string | null => {
      const ms = toMillisFlexible(v);
    ...
    if (deviceId) {
      ...
      const candidates = qSnap.docs.map((d) => ({ id: d.id, ...(d.data() as any) }));
      const model = toLower(modelRaw);
      const name = toLower(nameRaw);

      let filtered = candidates;
      if (model) {
        filtered = candidates.filter((d: any) => {
          const models = [d.deviceModel, d.model, d.modelNumber]
            .filter(Boolean)
            .map((x: any) => toLower(String(x)));
    ...
    const warranty =
      normalizeWarranty(d.warrantyExpiry) ||
 ...
 */
export const onSupportTicketCreatedGen1 = functions.firestore
