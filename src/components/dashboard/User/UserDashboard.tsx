...
const UserDashboard: React.FC<UserDashboardProps> = ({ userName }) => {
        ...
        const devices = await Promise.all(userDevicesSnapshot.docs.map(async (userDeviceDoc) => {
          ...
          try {
            ...
            if (deviceDoc.exists()) {
              ...
              const serialsArr = Array.isArray((deviceData as any)?.serials) ? (deviceData as any).serials : [];
              const warrantyList = serialsArr
                .filter((s: any) => s && s.warrantyExpiry)
                .map((s: any) => ({
                  serialNumber: s.serialNumber || s.serial || '',
                  warrantyStart,
                  warrantyEnd: s.warrantyExpiry as string,
                }));
              let warrantyEnd = warrantyList[0]?.warrantyEnd || null;
              if (!warrantyEnd) {
  ...
  const userDevices: Device[] = userDeviceOptions.map((device, index) => ({
    ...
    warrantyList: (device as any).warrantyList ?? [],
  }));
                          ...
                          <td className="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                            <div className="text-[12px] leading-tight text-gray-600 dark:text-gray-300">
                              {Array.isArray((device as any).warrantyList) && (device as any).warrantyList.length > 1 ? (
                                ...
                                <span className="text-gray-500 dark:text-gray-400">{device.warranty || 'N/A'}</span>
                              )}
                            </div>
                          </td>
                        </tr>
