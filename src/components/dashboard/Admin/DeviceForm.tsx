...
export default function DeviceForm() {
  ...
  const [showDeleteConfirm, setShowDeleteConfirm] = useState<{show: boolean, deviceId: string | null}>({show: false, deviceId: null});
  const [showAddConfirm, setShowAddConfirm] = useState(false);
  const [deviceToAdd, setDeviceToAdd] = useState<DeviceDoc | null>(null);

  const removeDevice = useCallback(async (id: string) => {
    setShowDeleteConfirm({show: true, deviceId: id});
  }, []);

  const confirmDelete = useCallback(async () => {
    if (!showDeleteConfirm.deviceId) return;
    await deleteDoc(deviceDoc(db, showDeleteConfirm.deviceId));
    setShowDeleteConfirm({show: false, deviceId: null});
  ...
  const handleAddDevice = useCallback((device: DeviceDoc) => {
  ...
  // Add these modal components before the main return
...
{/* Device Documentation */}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to add ${deviceToAdd?.name || 'this device'}?`}
